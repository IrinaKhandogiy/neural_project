...
class _ResultsState extends State<ResultsPage> {
  ...
  void _navigateToElementScreen(BuildContext context, int index) {
    Navigator.of(context).push(MaterialPageRoute(
        builder: (context) => ResultDetailsPage(
            title: 'Neural App | Result',
            reference: _experiments[index].reference)));
  }
  ...
  getResults() async {
    ...
    var user = await firestoreInstance
        ...
        .get();
    _user = entities.User(
        user.docs.isEmpty ? "" : user.docs[0].data()["uid"],
    ...
    await firestoreInstance
        ...
        .then((value) => value.docs.forEach((element) {
              ...
              element["doctor"].get().then((value) {
                ...
                _experiments.add(entities.Experiment(
                    element.reference, element["date"], doctor, _user!));
              });
            }));
    await firestoreInstance
        .collection("result")
  ...
  setResultDates(value, date) {
    var experiment = _experiments
        .firstWhereOrNull((element) => element.date == value["date"]);
    if (null != experiment) {
  ...
  filterResults(bool set_filter) {
    ...
    if (set_filter) {
      _filtered_experiments =
          _experiments.where((element) => element.resultDate != null).toList();
    } else {
      _filtered_experiments = [..._experiments];
    }
    if (selectedDate != null) {
      var format = DateFormat("dd.MM.yyyy");
      _filtered_experiments = _filtered_experiments
          .where((element) =>
                    ...
                    child: ListView.builder(
                        ...
                        itemBuilder: (context, index) {
                          return GestureDetector(
                              onTap: () {
                                _navigateToElementScreen(context, index);
                              },
                              child: Container(
                                padding: EdgeInsets.only(left: 22, right: 11),
                                height: 80,
                                margin:
                                    EdgeInsets.only(top: index != 0 ? 39 : 0),
                                ...
                                child: Row(
                                  ...
                                  children: [
                                    ...
                                    Spacer(),
                                    Text(
                                      _filtered_experiments[index].resultDate ==
                                              null
                                          ? "Результат\n не готов"
                                      ...
                                      style: GoogleFonts.openSans(
                                          ...
                                          fontSize: _filtered_experiments[index]
                                                      .resultDate ==
                                                  null
                                              ? 15
                                              : 13,
